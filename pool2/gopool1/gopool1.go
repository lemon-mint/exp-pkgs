...
func tworker[T any](pool *GoPool1[T], w *timedworker[T]) {
	for v := range w.ch {
		pool.handler(v)
		if pool.free(w) {
			return
...
func (pool *GoPool1[T]) start(preheat int) {
...
func (pool *GoPool1[T]) free(w *timedworker[T]) (stop bool) {
	w.tt = time.Now().UnixNano()
	pool.mu.Lock()
	if pool.maxWorkers == -1 {
		pool.workers--
...
func (pool *GoPool1[T]) Run(v T) (ok bool) {
	var w *timedworker[T]
	pool.mu.Lock()
...
func New[T any](maxWorkers int64, handler func(T), idleTimeout, gcPeriod time.Duration, preheat int) (*GoPool1[T], error) {
	if maxWorkers <= 0 {
...
func (pool *GoPool1[T]) Workers() int64 {
	pool.mu.Lock()
	w := pool.workers
	pool.mu.Unlock()
	return w
